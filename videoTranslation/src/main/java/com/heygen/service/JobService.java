    ...
    public class JobService {
        ...
        public Map<String, String> getJobStatus(String jobId) {
            ...
            // Assumption: If the elapsed time exceeds the timeout but is within twice the timeout, the job is considered completed.
            if ("pending".equals(job.getStatus())) {
                if (elapsedTime > job.getTimeout() && elapsedTime <= 2 * job.getTimeout()) {
                    job.setStatus("completed");
                }
                // Assumption: If the elapsed time exceeds twice the timeout, the job is deemed to have failed.
